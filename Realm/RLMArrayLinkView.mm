...
- (RLMArrayLinkView *)initWithParent:(__unsafe_unretained RLMObjectBase *const)parentObject
    ...
    if (property.type == RLMPropertyTypeObject) {
        return [self initWithList:realm::List(realm->_realm, parentObject->_row.get_linklist(col))
                            realm:realm
                       parentInfo:parentObject->_info
                         property:property];
    }
    return [self initWithList:realm::List(realm->_realm, parentObject->_row.get_subtable(col))
                        realm:realm
                   parentInfo:parentObject->_info
                     property:property];
}
